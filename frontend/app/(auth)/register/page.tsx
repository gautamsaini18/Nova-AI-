...
import { Mic, Mail, Lock, User, Eye, EyeOff, ArrowRight, Loader2, Check } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    toast.success("Account created! Welcome to Nova AI 🎙️");
    router.push("/dashboard");
  };

  const strength = PASSWORD_RULES.filter(r => r.test(password)).length;
  const strengthColors = ["#FF6B9D", "#FFD700", "#22c55e"];
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-2xl font-bold font-display gradient-text">Join Nova AI</h1>
        <p className="text-white/40 text-sm mt-1">Create your free account today</p>
      </div>

      <div className="glass rounded-2xl p-6 border border-white/[0.08]">
        {/* Google */}
        <button className="w-full flex items-center justify-center gap-3 py-3 rounded-xl glass-hover border border-white/10 text-sm font-medium mb-5 transition-all hover:border-white/20">
        ...
        <div className="relative mb-5">
          ...
          <div className="relative flex justify-center text-xs text-white/30">
            <span className="px-2" style={{ backdropFilter: "blur(20px)", background: "var(--glass-bg)" }}>or with email</span>
          </div>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Gautam Saini" className="input pl-9" />
            </div>
          </div>
                ...
                <div className="flex justify-between">
                  <div className="space-y-0.5">
                    {PASSWORD_RULES.map(rule => (
                      <div key={rule.label} className={`flex items-center gap-1.5 text-xs transition-colors ${rule.test(password) ? "text-green-400" : "text-white/30"}`}>
                        <Check size={10} className={rule.test(password) ? "opacity-100" : "opacity-0"} />
                        {rule.label}
                      </div>
                    ))}
                  </div>
                  <div className="text-xs font-medium" style={{ color: strengthColors[strength - 1] || "transparent" }}>
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            <div
              ...
              className={`w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0 mt-0.5 transition-all border ${
                agreed ? "bg-violet-500 border-violet-500" : "border-white/20 bg-transparent"
              }`}
            >
              {agreed && <Check size={11} className="text-white" />}
            </div>
            <span className="text-xs text-white/50 leading-relaxed">
              I agree to Nova AI's{" "}
          ...
          <button type="submit" disabled={loading || !agreed} className="btn-primary w-full py-3 disabled:opacity-60 disabled:cursor-not-allowed">
            {loading ? (
